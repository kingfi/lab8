...
module MakeInterval (Endpoint : ORDERED_TYPE) =
  struct
    ...
    let contains (intvl : interval) (x : Endpoint.t) : bool =
      match intvl with
      | Empty -> false
      | Interval (low, high) -> not ((Endpoint.compare x low < 0) || (Endpoint.compare x high > 0))
    ...
    let intersect (intvl1 : interval) (intvl2 : interval) : interval =
      ...
      | Empty, i2 -> Empty
      | i1, Empty -> Empty
      | Interval (low1, high1), Interval (low2, high2) -> if contains intvl1 low2 then
                                                            if contains intvl1 high2 then
                                                            Interval (low2, high2)
                                                            else Interval (low2, high1)
                                                          else if contains intvl2 low1 then
                                                            if contains intvl2 high1 then
                                                            Interval (low1, high1)
...
module IntInterval = MakeInterval( struct type t = int
                                  let compare = compare
                                  end);;
...
let intvl2 = IntInterval.create 12 400 ;;
let intvl1_intersect_intvl2 = IntInterval.intersect intvl2 intvl1 ;;
